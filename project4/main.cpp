...
void compress(std::ifstream &input, std::ofstream &output, std::unordered_map<std::size_t, std::string> &dict, int debug){
    ...
    while(!input.eof()){
        ...
        if(word != ""){
            int found = 0;
            for (std::unordered_map<std::size_t, std::string>::iterator itr = dict.begin(); itr != dict.end(); ++itr) {
                if (itr->second == word){
                    key = itr->first;
                    found = 1;
                }
...
void delta(std::string msg = ""){
    ...
    del = (high_resolution_clock::now() - start_time);
    std::cout << msg << del.count() << std::endl;
    start_time = high_resolution_clock::now();
}
