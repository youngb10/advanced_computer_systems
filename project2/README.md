...
One caveat is that the more threads utilized, the worse the compression will be. Compression algorithms work by exploiting patterns in the data. Larger data means more patterns can be found which improves compression. Distributing the input file over multiple threads reduces the algorithms ability to detect these patterns as it is only seeing a small piece of the picture. The compression is still valid; zstd can successfully decompress the compressed file with no losses. This is because there are multiple legal ways to compress a file, however there is only one optimal way. A single thread will see the most amount of data, and will result in the highest compression ratio. As the number of threads increase, the compression speeds up but increases the output file size. As the data shows, above 6 threads shows marginal gains in terms of speed. 
